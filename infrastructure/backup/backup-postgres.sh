...
BACKUP_FILE="${BACKUP_DIR}/rei_${TIMESTAMP}.sql.gz"
...
# Prune old backups
find "$BACKUP_DIR" -name "rei_*.sql.gz" -mtime +"$RETENTION_DAYS" -delete
REMAINING=$(find "$BACKUP_DIR" -name "rei_*.sql.gz" | wc -l | tr -d ' ')
